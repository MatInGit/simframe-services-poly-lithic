...
class KafkaListener(threading.Thread):
    ...
    def request_job_status(self, job_id: str):
        ...
        message = {"job_id": job_id, "action": "get_status"}

        try:
            self.producer.produce(
                "simframe_requests", key=job_id, value=json.dumps(message)
            )
            self.producer.flush()
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"🎧 Kafka listener started on {self.kafka_broker}, topic: {self.response_topic}"
            )
            ...
            while self.running:
                ...
                try:
                    ...
                    if job_id:
                        ...
                        if status == "completed":
                            print(
                                f"✅ Kafka notification: Job {job_id[:8]}... status={status}"
                            )
                        else:
                            print(
                                f"📨 Kafka notification: Job {job_id[:8]}... status={status if status else 'unknown'}"
                            )
...
def submit_job(
    ...
    try:
        response = requests.post(
            f"{wrangler_url}/submit_lattice/{model_id}", json=job_data, timeout=5
        )
        response.raise_for_status()
...
def verify_results(
        ...
        for job_id in list(pending_jobs):
            ...
            try:
                response = requests.get(
                    f"{wrangler_url}/get_result/{job_id}", timeout=5
                )
                ...
                if response.status_code == 200:
                    ...
                    if kafka_listener and job_id not in kafka_requested_jobs:
                        print(
                            f"✅ Job {job_id[:8]}... completed ({elapsed:.1f}s) - requesting via Kafka"
                        )
                    ...
                    if returned_beam:
                        ...
                        print(
                            f"   📊 Result contains beam data with {len(returned_beam)} properties"
                        )
                    else:
                    ...
                    verified_jobs[job_id] = {
                        ...
                        - submitted_jobs[job_id]["submitted_at"],
    ...
    if kafka_listener and len(kafka_listener.messages) > 0:
        if format_valid_count == len(requested_and_received):
            ...
            print(
                f"⚠️  {len(requested_and_received) - format_valid_count} messages with invalid format"
            )
...
def main():
    ...
    if args.randomize:
        ...
        print(
            f"🎲 Randomly distributing {total_jobs} jobs across {len(models)} model(s)\n"
        )
        ...
        for i in range(total_jobs):
            ...
            print(
                f"📤 Job {i + 1}/{total_jobs} -> {model_name[:20]}... ",
                end="",
                flush=True,
            )
        ...
        for idx, (model_id, model_info) in enumerate(model_list):
            ...
            num_jobs_for_this_model = jobs_per_model_target + (
            ...
            for i in range(num_jobs_for_this_model):
                job_count += 1
                print(
                    f"   Job {i + 1}/{num_jobs_for_this_model} (total: {job_count}/{total_jobs})... ",
                    end="",
                    flush=True,
                )

                result = submit_job(
                    args.wrangler_url, model_id, model_name, submitted_jobs
                )
    ...
    if len(models) > 1:
        ...
        for model_id, count in jobs_per_model.items():
            ...
            print(
                f"  {model_name[:25]:<25} {count:>3} jobs ({percentage:>5.1f}%) {bar}"
            )
    ...
    if successful_jobs > 0 and submitted_jobs:
        ...
        if verification_results["completion_times"]:
            avg_time = sum(verification_results["completion_times"]) / len(
                verification_results["completion_times"]
            )
        ...
        if kafka_listener:
            ...
            kafka_match_rate = (
                (kafka_matched / kafka_requested * 100) if kafka_requested > 0 else 0
            )
            format_valid_rate = (
                (format_valid / kafka_matched * 100) if kafka_matched > 0 else 0
            )
            ...
            print(
                f"  Valid format: {format_valid}/{kafka_matched} ({format_valid_rate:.1f}%)"
            )

            if verification_results["missing_notifications"] > 0:
                print(
                    f"  ⚠️  {verification_results['missing_notifications']} requests without response"
                )
        ...
        if kafka_listener:
            ...
            kafka_ok = (kafka_matched == kafka_requested) and (
                format_valid == kafka_matched
            )
        ...
        if all_verified and kafka_ok:
            ...
            if kafka_listener:
                ...
                print(
                    "   ✓ All Kafka messages have correct format: {'job_id': <id>, 'status': 'completed'}"
                )
        else:
            print("⚠️  Some tests failed - review results above")
            if not all_verified:
                print(
                    f"   ✗ {successful_jobs - verified_count} jobs failed verification"
                )
            if kafka_listener and not kafka_ok:
                missing = verification_results["missing_notifications"]
                invalid_format = (
                    kafka_matched - format_valid if kafka_matched > format_valid else 0
                )
                if missing > 0:
